...
export default function ZikrDetailScreen({ route }) {
  ...
  const successCooldownRef = useRef(0);
  ...
  const recorder = useAutoRecorder(zikr.id, async (uploadRes) => {
    try {
      ...
      const res = await pollRecordingStatus(recId);

      if (res?.status === "DONE") {
        successCooldownRef.current = Date.now() + SUCCESS_COOLDOWN_MS;

        // показываем ровно то, что посчитал воркер
        const delta = Math.max(1, Math.min(Number(res.repeats) || 1, 5));
        setStatusMsg(`Принято! +${delta}`);

        // и сразу синхронизируем прогресс с сервером
  ...
  useEffect(() => {
    ...
    if (!serverToday.completed) completedRef.current = false;
  ...
  async function refreshByZikrToday(retries = 2, delayMs = 400) {
  ...
  async function pollRecordingStatus(id) {
    ...
    for (const ms of delays) {
      try {
        ...
        if (s === "DONE" || s === "FAILED") {
          return { status: s, repeats: r?.data?.repeats ?? null };
        }
      } catch {}
...
const styles = StyleSheet.create({
  ...
  counters: { marginTop: spacing.lg, alignItems: "center" },
