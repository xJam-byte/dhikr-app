...
# VAD и распознавание
DEFAULT_VAD_MIN_SIL_MS = int(os.getenv("VAD_MIN_SIL_MS", "140"))
BEAM_SIZE               = int(os.getenv("ASR_BEAM_SIZE", "5"))
TEMPERATURE             = float(os.getenv("ASR_TEMPERATURE", "0.2"))
NO_SPEECH_THRESHOLD     = float(os.getenv("ASR_NO_SPEECH_THRESHOLD", "0.55"))
...
# Порог для отбора слов в words
WORD_MIN_PROB = float(os.getenv("ASR_WORD_MIN_PROB", "0.0"))

app = FastAPI(title="Dhikr ASR", version="1.3.1")
...
async def transcribe(
  ...
  lang: str = Form("auto"),
  vad_min_sil_ms: int = Form(DEFAULT_VAD_MIN_SIL_MS),
):
  ...
  try:
    ...
    for seg in segments_gen:
      ...
      if avg_lp is not None:
        c = max(0.0, min(1.0, 1.0 + float(avg_lp)))  # logprob -> [0..1]
        confs.append(c)
